...
int main(void) {
    ...
    EGLint vid;
    eglGetConfigAttrib(glDisplay, glConfig, EGL_NATIVE_VISUAL_ID, &vid); ///don't know if I need or not
	...
	glContext = eglCreateContext(glDisplay, glConfig, EGL_NO_CONTEXT, contextAttributes);
	glSurface = eglCreateWindowSurface(glDisplay, glConfig, (EGLNativeWindowType)sysInfo.info.x11.window, NULL); // X11?
	...
	fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV; /// V4L2_PIX_FMT_BGRX32  a yuv format might be faster but needs ISP unit?
    ...
    //EGL_DMA_BUF_PLANE0_PITCH_EXT, camGL->params.height * 3,
    //EGL_NONE
	...
	dma_image = eglCreateImageKHR(	  
					...
					(EGLint[])
					{
						...
						EGL_LINUX_DRM_FOURCC_EXT, DRM_FORMAT_YUYV,  /// takes 16 or 32 bits per pixel (or 8 probably)
